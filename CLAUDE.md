...
- **Program-page SEO (2026-07-20):** each program in `programs.ts` carries per-program `metaTitle` (distinct search query per class: Foundation = "online debate classes for beginners", Competition = "debate team training", 1-on-1 = "private world schools debate coaching", Bootcamp = "world schools debate summer camp") + `metaDescription` (payload-first, ≤~155 chars) + optional `seoH1` (keyword-bearing H1 on the detail page only; cards/steppers/checkout keep `name`) + `termDates` (ISO start/end feeding Course JSON-LD `CourseInstance`). `CourseJsonLd` now emits `offers.category: 'Paid'` (required for the Course rich result), start/end dates, `courseWorkload`/`courseSchedule` from `instruction`/`sessionLength`, and `instructor` Person nodes (page passes its `coaches`); the real charged price only — never `compareAt`/early-bird framing in schema. `/programs` index cards render the short `description`; `longDescription` is unique to the detail page (duplicate-content guard — don't put it back on the index). Every blog post's `BlogPostShell` CTA deep-links its most relevant program via `ctaHref`/`ctaLabel`. **Seasonal flip (do in September):** the bootcamp page hard-codes "August 2026 / last cohort / enrolling now" across copy + metaDescription + term banner — after the season, keep the URL alive (it accrues "debate summer camp" equity) and flip it to next-summer waitlist copy; never delete or 404 it.
- **Motion Bank (added 2026-07-23, strategy in `docs/motion-bank-strategy.md`):** 12,416 real tournament motions (1994–2026) compiled from public records (MIT hello-motions dataset + Tabbycat API harvest + official WSDC archive). Generated data: `src/data/motion-bank.json` (server import) and `public/motion-bank-core.json` + `public/motion-bank-infoslides.json` (client explorer, split so info slides lazy-load). **Never hand-edit the JSON** — rebuild via `python3 scripts/motion-bank/build_bank.py` (inputs + refresh procedure in its docstring). Types/topic metadata/helpers in `src/lib/motion-bank.ts` (17 topics with hand-written intros + coach notes — that copy is the E-E-A-T differentiator, keep it human). Routes: `/motions` (landing + `MotionExplorer` client search), `/motions/[topic]` (17 SSG pages, `dynamicParams=false`), `/motions/year/[year]` (2011+ only), `/motions/wsdc` (flagship Worlds archive, hosts hardcoded in page). All server-rendered with full motion text in HTML — this is the entire SEO edge over the SPA competitors (hellomotions etc. serve zero crawlable motions), so never convert these pages to client-side rendering. Wired into sitemap.ts, llms.txt, Navbar, Footer, /resources promo. Motion text is verbatim from sources; never "clean up" or invent motions. Static "12,000+" counts in metadata/nav need a manual bump when thresholds cross; everything else computes from data.
