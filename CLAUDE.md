...
- **What's sold** lives in `src/data/programs.ts` → `Program.enrollment` (`unitLabel` + `amount`). Prices are launch **placeholders pending owner sign-off**, incl. 1-on-1 sold as a 5-session pack ($545 = 5 × $109) because hourly billing doesn't fit fixed-price checkout. Prices are ALWAYS re-resolved server-side from this file — never trust client amounts. `GRADE_LEVELS` there is the shared grade allow-list. **Only 3 of the 4 programs are purchasable:** National Team Sprint is `invitationOnly` — no cart line, an "Request consideration" CTA to `/contact` instead, and the payment-intent route hard-rejects it (`program.invitationOnly` guard) so a crafted request can't buy in.
...
- [ ] **Pricing sign-off** — amounts in `src/data/programs.ts` are placeholders set against market anchors (~$100/hr 1-on-1; $2,850–$4,149 two-week camps). Foundation $549/term, Competition Team $749/term, 1-on-1 $109/hr (sold as a 5-session pack at $545), Summer Bootcamp $299 (8 hours / 4 sessions, dates Aug 18–27 2026 — all placeholder). National Team Sprint is invitation-only with **no price** (needs a decision if invited students are billed). **Checkout sells the three group/1-on-1 units directly.** Junior and Senior bands currently share one placeholder price each — if they should differ, the cart needs a band SKU (today band is a scheduling preference confirmed by the coach, not a price variant).
- [ ] **Program content sign-off (owner)** — schedules/term dates are owner-supplied and wired in verbatim, but the **Junior/Senior age bands (11–14 / 14–18) are placeholders** (owner gave no ages); confirm or adjust. All four pages now carry the deep sections (session flow / what's included / FAQ); **the session-flow timings, deliverables, and FAQ commitments on all of them are invented and need owner review** (esp. make-up policy, tournament-support scope, the Sprint's "two classes + weekly practice" and its 2-hour session assumption). Foundation's copy was also nudged from a 90-min to a 2-hour class — reread for accuracy.
